...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initGame(reviewMode = false) {
        ...
        // Initialize gesture recognition if there are cards
        if (cards.length > 0) {
            initHammer();
        }
    ...
    function createCard(data) {
        ...
        // Add draggable areas on the sides
        const leftHandle = document.createElement('div');
        leftHandle.className = 'drag-handle drag-handle-left swipe-handle';
        leftHandle.innerHTML = '<i class="fas fa-chevron-left"></i>';
        
        const rightHandle = document.createElement('div');
        rightHandle.className = 'drag-handle drag-handle-right swipe-handle';
        rightHandle.innerHTML = '<i class="fas fa-chevron-right"></i>';
        
        // Card header is also a drag handle
        const cardHeader = document.createElement('div');
        cardHeader.className = 'card-header swipe-handle';
        ...
        // Card footer is also a drag handle
        const cardFooter = document.createElement('div');
        cardFooter.className = 'card-footer swipe-handle';
    ...
    function initHammer() {
        ...
        // Get the content area - we want to prevent swiping when user interacts with this
        const contentArea = currentCard.querySelector('.card-content');
        
        // Track whether a touch started in the content area
        let touchStartedInContent = false;
        
        // Add touch event listeners to the card content area to detect scrolling
        contentArea.addEventListener('touchstart', function(e) {
            touchStartedInContent = true;
            touchStartY = e.touches[0].clientY;
            isScrolling = false;
        }, { passive: true });
        
        contentArea.addEventListener('touchmove', function(e) {
            if (touchStartedInContent) {
                const touchY = e.touches[0].clientY;
                const deltaY = Math.abs(touchY - touchStartY);
                
                // If vertical movement is significant, user is trying to scroll
                if (deltaY > 10) {
                    isScrolling = true;
                }
            }
        ...
        hammerManager.on('pan', function(e) {
            // If we're scrolling content, don't process pan events
            if (isScrolling) return;
            
            // Check if the touch/click target is inside the content area
            const target = e.target;
            if (target && !target.closest('.swipe-handle') && !contentArea.contains(target)) {
                // If we're not in a swipe handle or the content area, don't process
                return;
            }
        ...
        hammerManager.on('swiperight swipeleft', function(e) {
            // Don't process swipes if we're scrolling
            if (isScrolling) return;
            
            // Check if the swipe started in the content area
            if (touchStartedInContent) return;
        ...
        hammerManager.on('panend', function(e) {
            // Don't process if we're scrolling
            if (isScrolling) return;
            
            // Check if the pan started in the content area
            if (touchStartedInContent) {
                // Just reset the card position
    ...
    function goToNextCard() {
        setTimeout(() => {
            ...
            if (currentCardIndex < cards.length) {
                ...
                // Initialize hammer on new current card
                initHammer();
    ...
    // Get a shortened category name for display
    function getShortCategoryName(category) {
        const nameMap = {
            "Models of Disability": "Models",
            "Visual Disabilities": "Visual",
            "Auditory Disabilities": "Auditory",
            "Deaf-Blindness": "Deaf-Blind",
            "Speech and Language Disabilities": "Speech",
            "Mobility, Flexibility, and Body Structure Disabilities": "Mobility",
            "Cognitive Disabilities": "Cognitive",
            "Seizure Disabilities": "Seizure",
            "Psychological Disabilities": "Psychological",
            "Multiple/Complex Disabilities": "Multiple"
        };
